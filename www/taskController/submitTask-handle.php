...
<?php 
    ...
    $employeeNote = '. ' . 'Employee note:' . ' ' . $taskDescription;
    $thisDay = date("Y-n-j"); 
	...
	$sql = "UPDATE task SET TaskStatus = '3', TaskDescription = '$employeeNote', DateEmployeeSubmit = '$thisDay' WHERE TaskID = '$editID'";
	$result = $conn->query($sql);

	if (!$conn->query($sql)){
        ?>
        <div class="d-flex justify-content-center mt-5">
            <h3 class="alert alert-danger d-flex justify-content-center w-50 p-50">Can not submit task.</h3>
        </div>
        <?php
        die('Can not submit task: ' . $conn->error);
    }else{
        ?>
        <div class="d-flex justify-content-center mt-5">
            <h3 class="alert alert-success d-flex justify-content-center w-50">Task submited.</h3>
        </div>
        <?php
    }
