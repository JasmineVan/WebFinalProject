...
<?php 
	...
	$sql = "UPDATE task SET TaskStatus = '4' WHERE TaskID = '$rejectID'";
	$result = $conn->query($sql);

	if (!$conn->query($sql)){
        ?>
        <div class="d-flex justify-content-center mt-5">
            <h3 class="alert alert-danger d-flex justify-content-center w-50 p-50">Can not reject task.</h3>
        </div>
        <?php
        die('Can not reject task: ' . $conn->error);
    }else{
        ?>
        <div class="d-flex justify-content-center mt-5">
            <h3 class="alert alert-success d-flex justify-content-center w-50">Task rejected.</h3>
        </div>
        <?php
    }
