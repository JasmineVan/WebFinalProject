...
<?php 
	...
	$sql = "UPDATE task SET TaskStatus = '2' WHERE TaskID = '$startID'";
	$result = $conn->query($sql);

	if (!$conn->query($sql)){
        ?>
        <div class="d-flex justify-content-center mt-5">
            <h3 class="alert alert-danger d-flex justify-content-center w-50 p-50">Can not start task.</h3>
        </div>
        <?php
        die('Can not start task: ' . $conn->error);
    }else{
        ?>
        <div class="d-flex justify-content-center mt-5">
            <h3 class="alert alert-success d-flex justify-content-center w-50">Task in progress.</h3>
        </div>
        <?php
    }
