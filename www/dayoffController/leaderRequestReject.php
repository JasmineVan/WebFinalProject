...
<body>
    ...
    <nav class="navbar navbar-expand-sm bg-dark navbar-dark">
        <ul class="navbar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="../leaderController/leaderDayoffManagement.php">Dayoff Management</a>
            </li>
        </ul>
	...
	$sql = "UPDATE dayoff SET RequestStatus = '3', Temp = '0' WHERE DayOffID = '$requestID'";
	$result = $conn->query($sql);

	if (!$conn->query($sql)){
        ?>
        <div class="d-flex justify-content-center mt-5">
            <h3 class="alert alert-danger d-flex justify-content-center w-50 p-50">Can not reject request.</h3>
        </div>
        <?php
        die('Can not reject request: ' . $conn->error);
    }else{
        ?>
        <div class="d-flex justify-content-center mt-5">
            <h3 class="alert alert-success d-flex justify-content-center w-50">Request rejected.</h3>
        </div>
        <?php
    }
    ?>
